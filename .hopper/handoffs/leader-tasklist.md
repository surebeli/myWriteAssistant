...
| **T17** | Builder (GPT-5.5) | API route 集成测试 scaffolding | 选 vitest + supertest（或等价）；新增 `tests/integration/api/` 目录；提供 mock provider helper（不真调上游） | scaffolding 跑通；写 1 条 sanity 测试（POST /api/chat with mock providerConfig → adapter 被调）；T02 / T05 / AC16 等后续测试可在此基础上加 | T01 | M |
